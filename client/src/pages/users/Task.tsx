...
const Task = () => {
  ...
  const fetchUserAndTasks = async () => {
    try {
      ...
      const tasksResponse = await axios.get(`http://localhost:3000/api/users/${parsedDataUser.id}/tasks`, {
        headers: { Authorization: `Bearer ${parsedDataUser.id}` }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTask) {
        ...
        const dataUser = localStorage.getItem('pendingUserData');
        const parsedDataUser = JSON.parse(dataUser || '{}');
        console.log('parsedDataUser', parsedDataUser.id);

        // Update existing task
        await axios.put(
          `http://localhost:3000/api/users/${parsedDataUser.id}/tasks/${editingTask.id}`,
          { status },
          { headers: { Authorization: `Bearer ${parsedDataUser.id}` } }
        );
      }
